...
from dfxutils.app import MeasurementStep

import cv2
import pygame
import numpy as np
from pygametest import DemoRenderer
...
_message_action = {
    ...
    MeasurementStep.MEASURING: "Peruuta painamalla Esc",
    MeasurementStep.WAITING_RESULTS: "Peruuta painamalla Esc",
    MeasurementStep.COMPLETED: "Poistu painamalla Esc",
    MeasurementStep.USER_CANCELLED: "",
    MeasurementStep.FAILED: "Mittaus epäonnistui, poistu painamalla Esc"
}

_message_state = {
    MeasurementStep.NOT_READY: "Ei valmiina",
    MeasurementStep.READY: "Valmiina mittaamaan",
    MeasurementStep.USER_STARTED: "Aloitetaan",
    MeasurementStep.MEASURING: "Mittaus käynnissä",
    MeasurementStep.WAITING_RESULTS: "Odotetaan tuloksia",
    MeasurementStep.COMPLETED: "Mittaus onnistui",
    MeasurementStep.USER_CANCELLED: "Mittaus peruutettu",
    MeasurementStep.FAILED: "Mittaus epäonnistui"
}


class PygameRenderer():
    def __init__(self, version, image_src_name, fps, app, screen, useDemorenderer = True, sf=1.0):
        self._render_queue = asyncio.Queue(1)
        self._demo_renderer = DemoRenderer(app,screen)
        ...
        self._useDemoRenderer = useDemorenderer
    ...
    def setResultsCount(self, results_count):
        self._demo_renderer.SetResults(results_count)
    ...
    async def render(self):
        ...
        while not self._rendering_last:
            try:
                ...
                render_image_copy = cv2.cvtColor(render_image_copy, cv2.COLOR_BGR2RGB)   # cv2 is BGR, pygame expects RGB
                render_image_copy = np.rot90(render_image_copy)                          # optional: rotate if needed
                frame_surface = pygame.surfarray.make_surface(render_image_copy)
                #self._draw_on_image(render_image_copy, meta)
                if self._useDemoRenderer:
                    self._demo_renderer.RenderSurface(frame_surface)
                else:
                ...
                if self._app.step == MeasurementStep.READY:
                    self._app.step = MeasurementStep.USER_STARTED
                elif self._app.step == MeasurementStep.FAILED:
    ...
    def _draw_on_image(self, render_image, image_meta):
        ...
        if self._app.step == MeasurementStep.MEASURING:
            if self._app.begin_frame > 0:
                ...
                r = self._draw_text(f"Valmiina {frame_number} / {self._app.end_frame + 1}", render_image,
                                    (c, r))
        elif self._app.step == MeasurementStep.WAITING_RESULTS and showDebugInfo:
            if self._app.begin_frame > 0:
                r = self._draw_text(
        ...
        if self._sent_chunk is not None and showDebugInfo:
            ...
            r = self._draw_text(f"{msg} chunk: {self._sent_chunk + 1} of {self._app.number_chunks}", render_image,
                                (c, r))
        if self._results:
            r = self._draw_text(f"Tulokset: {self._recv_chunk + 1} / {self._app.number_chunks}", render_image,
                                (c, r))
